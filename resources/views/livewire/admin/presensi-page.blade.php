                ...
                <div class="card-body">
                    ...
                    @if($selectedJadwal)
                        ...
                        <div class="text-center">
                            ...
                            <button type="button" class="btn btn-success me-2" wire:click="initializePresensiForJadwal({{ $selectedJadwal }})">
                                <i class="mdi mdi-account-multiple-plus me-1"></i> Inisialisasi Presensi
                            </button>
                            @if(count($presensiList) > 0)
                                <button type="button" class="btn btn-primary" wire:click="markAllPresent" wire:confirm="Apakah Anda yakin ingin menandai semua siswa sebagai hadir?">
                                    <i class="mdi mdi-check-all me-1"></i> Presensi Hadir Semua
                                </button>
                            @endif
        ...
        <div class="col-lg-7">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    @if(count($presensiList) > 0)
                        <!-- Tab Navigation -->
                        <ul class="nav nav-tabs mb-3" id="presensiTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="belum-presensi-tab" data-bs-toggle="tab" data-bs-target="#belum-presensi" type="button" role="tab">
                                    <i class="mdi mdi-clock-outline me-1"></i>
                                    Belum Presensi 
                                    <span class="badge bg-danger ms-1">{{ $belumPresensi->count() }}</span>
                                </button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="sudah-presensi-tab" data-bs-toggle="tab" data-bs-target="#sudah-presensi" type="button" role="tab">
                                    <i class="mdi mdi-check-circle me-1"></i>
                                    Sudah Presensi 
                                    <span class="badge bg-success ms-1">{{ $sudahPresensi->count() }}</span>
                                </button>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content" id="presensiTabsContent">
                            <!-- Belum Presensi Tab -->
                            <div class="tab-pane fade show active" id="belum-presensi" role="tabpanel">
                                @if($belumPresensi->count() > 0)
                                    <div class="list-group list-group-flush">
                                        @foreach($belumPresensi as $presensi)
                                            @include('livewire.admin.partials.presensi-item', ['presensi' => $presensi])
                                        @endforeach
                                    </div>
                                @else
                                    <div class="text-center py-4">
                                        <i class="mdi mdi-check-all text-success" style="font-size: 3rem;"></i>
                                        <h5 class="text-success mt-3">Semua Siswa Sudah Presensi!</h5>
                                        <p class="text-muted">Tidak ada siswa yang belum melakukan presensi</p>
                                    </div>
                                @endif
                            </div>

                            <!-- Sudah Presensi Tab -->
                            <div class="tab-pane fade" id="sudah-presensi" role="tabpanel">
                                @if($sudahPresensi->count() > 0)
                                    <div class="list-group list-group-flush">
                                ...
                                @else
                                    <div class="text-center py-4">
                                        ...
                                        <h5 class="text-muted mt-3">Belum Ada yang Presensi</h5>
                                        <p class="text-muted">Siswa belum melakukan presensi untuk jadwal ini</p>
                                    </div>
                                @endif
                            </div>
                        </div>
