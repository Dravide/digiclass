    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">Rekap Presensi Tata Usaha</h4>
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active">Rekap Presensi Tata Usaha</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <!-- Search Tata Usaha -->
                        <div class="col-md-3 mb-3">
                            <label class="form-label">Cari Tata Usaha</label>
                            <input type="text" wire:model.live.debounce.500ms="searchTataUsaha" class="form-control" placeholder="Nama atau username...">
                        </div>
        ...
        <div class="col-md-2">
            <div class="card widget-flat">
                <div class="card-body">
                    ...
                    <h5 class="text-muted fw-normal mt-0" title="Total Tata Usaha">Total Tata Usaha</h5>
                    <h3 class="mt-3 mb-3">{{ $statistik['total_tata_usaha'] ?? 0 }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-2">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="header-title mb-0">Data Rekap Presensi Tata Usaha</h4>
                    <div>
                ...
                <div class="card-body">
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    @foreach($rekapData as $index => $data)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $data['tata_usaha']->user->name ?? '-' }}</td>
                                            <td>{{ $data['tata_usaha']->user->username ?? '-' }}</td>
                                            <td>
                                                @if($data['tata_usaha']->jabatan)
                                                    <span class="badge bg-primary">{{ $data['tata_usaha']->jabatan }}</span>
                                                @else
                                            ...
                                            <td>
                                                @php
                                                    $tataUsahaId = $data['tata_usaha']->id;
                                                    if ($filterType === 'bulan') {
                                                        $tanggalMulai = Carbon\Carbon::createFromDate($selectedTahun, $selectedBulan, 1)->format('Y-m-d');
                                                        $tanggalSelesai = Carbon\Carbon::createFromDate($selectedTahun, $selectedBulan, 1)->endOfMonth()->format('Y-m-d');
                                                    } else {
                                                        $tanggalMulai = $this->tanggalMulai;
                                                        $tanggalSelesai = $this->tanggalSelesai;
                                                    }
                                                @endphp
                                                <a href="{{ route('admin.detail-presensi-tata-usaha', ['tataUsahaId' => $tataUsahaId, 'tanggalMulai' => $tanggalMulai, 'tanggalSelesai' => $tanggalSelesai]) }}" 
                    ...
                    @else
                        <div class="text-center py-4">
                            ...
                            <p class="text-muted">Belum ada data presensi tata usaha untuk periode yang dipilih.</p>
                        </div>
                    @endif
                </div>
            </div>
