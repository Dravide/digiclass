...
class GuruManagement extends Component
{
    ...
    public function store()
    {
        ...
        try {
            ...
            $this->resetForm();
            $this->dispatch('guru-created', 'Guru dan akun user berhasil ditambahkan! Password default: password123');
        } catch (\Exception $e) {
    ...
    public function generateAccount($guruId)
    {
        try {
            $guru = Guru::findOrFail($guruId);
            
            // Check if guru already has email
            if (empty($guru->email)) {
                $this->dispatch('guru-error', 'Guru harus memiliki email terlebih dahulu untuk generate akun!');
                return;
            }
            ...
            if ($existingUser) {
                $this->dispatch('guru-error', 'Akun dengan email ini sudah ada!');
                return;
            }
            ...
            $this->dispatch('guru-account-generated', 'Akun berhasil dibuat untuk guru ' . $guru->nama_guru . '! Password default: password123');
        } catch (\Exception $e) {
            $this->dispatch('guru-error', 'Gagal membuat akun: ' . $e->getMessage());
        }
    }

    public function render()
    {
        $query = Guru::with(['mataPelajaran', 'user'])
